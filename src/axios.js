...
// // 解密
// const decryptMsg = jasypt.decrypt('MHmX6tASyMGiHst/DvYtZg==');
...
axios.interceptors.request.use((config) => { //  config相当于options对象
  ...
  if ((method.toLowerCase() === 'post' || method.toLowerCase() === 'put') && data && typeof data === 'string') {
    ...
    config.paramsSerializer = function (params) {
      // 电子签名的时候使用
      if (params.password) {
        params.password = jasypt.encrypt(params.password);
      }
...
axios.interceptors.response.use(response => {
  ...
  if (response.data.state === 30003) {
    sessionStorage.clear()
    vm.$router.push('./login');
    setTimeout(() => { vm.$message.warning(response.message) }, 0)
  }
  if (response.data.state === 30004) {
    sessionStorage.clear()
    vm.$router.push('./login');
  }
